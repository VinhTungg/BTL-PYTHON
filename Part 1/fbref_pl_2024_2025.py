...
def extract_table_html_from_container(driver, container_id, expected_table_id):
    """
    FBref thường giấu bảng trong <!-- ... -->.
    Bóc cả trực tiếp lẫn comment để lấy table thật.
    """
    try:
...
def parse_table_html(table_html, page_tag):
    ...
    Với các trang khác: giữ nguyên header theo data-stat/text.
    """
    rows_data = []
...
def db_file_short(p):  # in log cho gọn
